class App {
    constructor(questions) {
        this.questions = questions.map(
            (q, index) => q.multi ? new MultiChoiceQuestion(q, index) : new SingleChoiceQuestion(q, index)
        );
    ...
    showResults() {
        ...
        this.questions.forEach((currentQuestion, questionNumber) => {
            ...
            switch (currentQuestion.multi) {
                case true:
                    userAnswer = [...answerContainer.querySelectorAll(selector)].map(answer => answer.value).join('') || '';
                    break;
                case false:
